...
def segment_image(image_array: np.ndarray, origin_image: Image.Image = None):
    # 连通分支切割
    subimages = segment_image_by_region(image_array, origin_image)
    if len(subimages) == 4:
        new_ = []
        for image in subimages:
            new_.append(add_padding(image))
        return new_
    # 没有找到四张小图像的情况，直接返回 None
    return None


def segment_image_by_region(image_array: np.ndarray,
                            origin_image: Image.Image = None) -> np.ndarray:
    """
...
def add_padding(img: np.ndarray) -> np.ndarray:
    # 创建一个边长为 len_ 的正方形
    len_ = 32
    square_ = np.ones((len_, len_))
    width_ = len_ - img.shape[0]
    height_ = len_ - img.shape[1]
    x_start = width_ // 2
    y_start = height_ // 2
    for x in range(img.shape[0]):
        for y in range(img.shape[1]):
            square_[x + x_start, y + y_start] = img[x, y]
    return square_
